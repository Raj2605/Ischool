<?php
  ...
  $STU_EMAIL = "";
	...
	if (isset($_POST["STU_EMAIL"]) && $_POST["STU_EMAIL"] != "") {
		$ATU_EMAIL = $_POST["STU_EMAIL"];
	}
...
?>  
   ...
   <div class="container">
    ...
    <?php
     ...
     <form method="post" action="">
     <div class="form-group row">
        <label class="offset-sm-3 col-form-label">Email: </label>
        <div>
          <input class="form-control mx-3" id="STU_EMAIL" tabindex="1" maxlength="20" size="20" name="STU_EMAIL" autocomplete="off" value="<?php echo $STU_EMAIL ?>">
        </div>
        <div>
          <input class="btn btn-primary mx-4" value="View" type="submit"	onclick="">
        </div>
    ...
    <?php
      ...
      { 
        $sql = "SELECT stu_email FROM courseorder";
        $result = $conn->query($sql);
        while($row = $result->fetch_assoc()){
          if($_POST["STU_EMAIL"] == $row["stu_email"]){ ?>
            <div class="row justify-content-center">
              <div class="col-auto">
                <h2 class="text-center">Payment Receipt</h2>
                <table class="table table-bordered">
